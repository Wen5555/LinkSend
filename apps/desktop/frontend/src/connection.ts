...
export function humanizeBackendError(raw: unknown): string {
  ...
  const byCode: Record<string, string> = {
    ...
    CLIPBOARD_WRITE_FAILED: '文字未能写入剪贴板，请稍后重试复制。',
    ...
    CONTENT_LIMIT_EXCEEDED: '内容超过大小或图片尺寸限制。',
    ...
    CONTENT_SNAPSHOT_UNAVAILABLE: '发送快照已不可用，请重新选择内容。',
    CONTENT_OWNERSHIP_MISMATCH: '发送快照的归属无法确认，请重新创建草稿。',
