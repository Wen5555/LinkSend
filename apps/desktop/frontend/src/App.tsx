...
export default function App() {
  ...
  const taskBusy=useRef(false), remoteBusy=useRef(false);
  ...
  const paired=devices.filter(d=>d.trusted&&d.id!==status?.identity);
  ...
  const run=async(k:string,fn:()=>Promise<unknown>,msg?:string)=>{
    if(op)return;
    setOp(k);setError('');
    try{await fn();if(msg)setNotice(msg);await loadTasks();await loadRemote()}
    catch(e){setError(humanizeBackendError(e))}finally{setOp('')}
  };
  ...
  const states=useMemo(()=>[
    ['服务可用',!!status?.ready],['信令可达',diag?.server_health==='ok'],['自动接收',!!inbox?.listening],['配对设备在线',paired.some(d=>d.online)],['直连已建立',tasks.some(t=>['connected','awaiting_acceptance','transferring','verifying'].includes(t.phase))]
  ...
  return <div className="app-shell">
    ...
    <section className="workspace"><header className="topbar"><div><p className="eyebrow">{tab==='transfer'?'工作台':tab==='devices'?'配对':'本地配置'}</p><h1>{tab==='transfer'?'传输':tab==='devices'?'设备':'设置与诊断'}</h1></div><div className="top-status"><span className={inbox?.listening?'status-dot ready':'status-dot'}/>{inbox?.listening?'可接收文件':'正在连接'}<button className="icon-button" aria-label="刷新" onClick={()=>{void loadTasks();void loadRemote()}}>↻</button></div></header>{error&&<div className="banner error-banner" role="alert"><span>!</span><p>{error}</p><button onClick={()=>setError('')} aria-label="关闭提示">×</button></div>}{notice&&<div className="banner notice-banner" role="status"><span>✓</span><p>{notice}</p><button onClick={()=>setNotice('')} aria-label="关闭提示">×</button></div>}{tab==='transfer'?<Transfer {...{states,paired,selected,peer,setPeer,paths,setPaths,receiveDir,active,pickFiles,pickDir,send,op,tasks,run,inbox}}/>:tab==='devices'?<Devices {...{devices,membership,invite,setInvite,joinToken,setJoinToken,run,op,prefs,status}}/>:<Settings {...{prefs,setPrefs,receiveDir,setReceiveDir,ifaces,diag,save,effective,prefsStatus,inbox}}/>}</section>
    {incoming&&<IncomingConfirm task={incoming} device={devices.find(d=>d.id===incoming.peer_id)} run={run} op={op}/>}
  </div>
}

function Transfer(p:any){return <div className="page-stack"><div className="connection-strip">{p.states.map(([l,ok]:[string,boolean])=><div className="connection-item" key={l}><span className={ok?'status-dot ready':'status-dot'}/><span>{l}</span><small>{ok?'已就绪':'等待中'}</small></div>)}</div><div className="transfer-grid"><section className="surface send-surface"><div className="section-heading"><div><span className="section-kicker">发送</span><h2>选择内容，直接发给对方</h2></div><span className="step-icon blue">↑</span></div><label className="field-label">发送给<select value={p.peer} onChange={(e:any)=>p.setPeer(e.target.value)}><option value="">选择已配对设备</option>{p.paired.map((d:app.DeviceInfo)=><option key={d.id} value={d.id}>{d.name||'未命名设备'} · {d.online?'在线':'暂时离线'}</option>)}</select></label>{p.selected&&<div className="peer-inline"><span className="avatar small">{(p.selected.name||'D')[0]}</span><div><strong>{p.selected.name||'未命名设备'}</strong><small>已配对 · {p.selected.online?'可以发送':'等待对方应用上线'}</small></div></div>}<div className="picker-row"><button className="secondary" onClick={p.pickFiles}>＋ 选择文件</button><button className="secondary" onClick={()=>p.run('pick-source',async()=>{const x=await Backend.PickSourceDirectory();if(x)p.setPaths([...p.paths,x])})}>＋ 选择文件夹</button></div><div className="file-list">{p.paths.length?p.paths.map((x:string,i:number)=><div className="file-row" key={x+i}><span className="file-icon">▧</span><span title={x}>{x.split(/[\\/]/).pop()}</span><button aria-label="移除" onClick={()=>p.setPaths(p.paths.filter((_:string,n:number)=>n!==i))}>×</button></div>):<div className="empty-picker"><span>＋</span><strong>选择要发送的文件或文件夹</strong><small>点击发送后，对方会收到确认弹窗</small></div>}</div><button className="primary full send-button" disabled={!!p.active||!p.peer||!p.paths.length||p.op==='send'} onClick={p.send}>{p.active?'当前传输完成后再发送':'发送给对方'}</button><p className="hint">{!p.peer?'请先选择一台已配对设备':!p.paths.length?'请选择至少一个文件或文件夹':'无需让对方提前点击“开始接收”'}</p></section><section className="surface receive-ready"><div className="section-heading"><div><span className="section-kicker">接收</span><h2>{p.inbox?.listening?'已准备好接收':'正在恢复接收'}</h2></div><span className={p.inbox?.listening?'receive-orb ready':'receive-orb'}>↓</span></div><p className="intro">应用打开时自动等待请求。收到文件后会弹出确认，不需要先点开始。</p><div className="receiver-status"><span className={p.inbox?.listening?'status-dot ready':'status-dot'}/><div><strong>{p.inbox?.listening?'后台接收已开启':'后台接收正在连接'}</strong><small>{p.inbox?.last_error?`最近状态：${p.inbox.last_error}`:'空闲时保持在线'}</small></div></div><label className="field-label">保存到<input value={p.receiveDir} readOnly placeholder="请选择接收目录"/></label><button className="secondary full" onClick={p.pickDir}>更改接收目录</button><div className="receive-note"><span>i</span><p>你可以在确认弹窗中为常用设备开启“以后自动接收”。</p></div></section></div><section className="tasks-section"><div className="section-heading compact"><div><span className="section-kicker">活动与记录</span><h2>任务</h2></div><span className="task-count">{p.tasks.length} 个</span></div>{p.tasks.length?<div className="task-list">{p.tasks.slice().reverse().map((t:app.TaskSnapshot)=><Task key={t.id} t={t} run={p.run}/>)}</div>:<div className="empty-state"><div className="empty-illustration">⇄</div><strong>这里会显示传输记录</strong><p>选择设备和文件后即可发送；接收无需提前操作。</p></div>}</section></div>}
