...
`content_v1` 复用 V1 的认证 QUIC、原始 Manifest 和块流。普通文件继续使用原 JSON/摘要与旧端双向兼容；内容类型不写入 Manifest，避免旧端忽略字段后重算出不同摘要。该能力限定一项普通文件：`content:{version:1,entry_id:0,kind,media_type,size,digest,width?,height?}`；`kind` 为 `text|url|image`，其中 digest 与原 entry.hash、内容快照 digest 均是小写 BLAKE3-256，size 必须完全一致。描述符不含正文、本地绝对路径或快照所有权 ID。
