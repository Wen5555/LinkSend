...
部署后的候选分支继续产生 `fcc48ebfee7ce34ae73567c9cf0ab9fd0f00cbfe`（任务暂停迟到进度修复）和后续 workflow/文档提交；这些提交没有修改 `cmd/rendezvous`、`internal/server`、`internal/signaling`、协议版本、服务配置或 schema。因此测试主站仍准确记录为从干净 commit `4b7ccf66…` 构建的产品 `0.2.0`，不是谎报为桌面候选 HEAD。以后产品版本变化或服务端依赖变化时按本页事务重新同步；纯桌面/文档提交不为追求 SHA 外观而无意义重启服务。
