...
当前源码和测试预发布版本为 `0.4.0`，协议保持 V1。里程碑状态以源码实现、自动化、物理实机和发布验证分别判定。
...
- M2 — **IMPLEMENTED / PASS（自动化与香港测试主站）**：身份、邀请、WSS presence、session/generation/owner、撤销和本地信任已落地；测试主站 schema 2 配对与完成/拒绝后立即新连接已复核。
