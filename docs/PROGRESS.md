...
- 目录选择器的受管复核已以 E5A Mac 0c59 同行的 `ghost` 设置按钮进入正确编辑器；“选择接收目录”由同一 4d package PID 拥有，树稳定后公开 Edit `1152` 的 `ValuePattern`、确认 Button `1` 的 `InvokePattern` 和取消 Button `2`。同一入口完成受管目录的 `ValuePattern` 写入和确认，但返回应用后未找到可 Invoke 的“保存设备偏好”，未落库；SQLite 保持目录/策略空、revision 7。早期未加 owner/就绪约束的不可达观察不再作为结论。
