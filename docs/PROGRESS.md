...
- Windows 125% DPI 原生小窗请求 960×640 后受最小高度约束为 960×700，继续请求 600×400 时钳制为 900×700，对应 720×560 逻辑最小值乘 1.25；进程 exit 0。延迟 Raw UIA 可读取 production DOM，39 字中文设备名 IsOffscreen=false 且边界留在窗口内，传输按钮声明 keyboard-focusable；但 PostMessage/真实键盘事件后的 FocusedElement 仍为空，准确包键盘自动化为 UNRESOLVED。当前系统 light theme 未改，150%/深色 NOT RUN。
- `387b57c` 与 M5 `d0c4a4b` 的隔离 loopback 控制面做了两个方向：旧 client 加入当前 server 返回 `VERSION_INCOMPATIBLE`，当前 client 加入旧 server 返回 `AUTHENTICATION_FAILED`；两端管理视图都只保留原管理员，没有幽灵成员。该 PARTIAL 只证明控制版本门闩，不是桌面升级或混合文件通过。
