...
Candidate exchange 测试覆盖发送失败、接收失败、缺少 `end_of_candidates`、父 context 取消和 malformed message，并使用确定的 done/context 同步点。阶段错误保留 `CANCELLED`、`CANDIDATE_EXCHANGE_TIMEOUT`、`CHECK_TIMEOUT` 和 QUIC handshake 分类，不能统一写成一个连接超时。
