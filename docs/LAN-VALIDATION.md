...
2026-09-11 已在物理 Windows（以太网）↔macOS arm64（Wi-Fi）完成双向 QUIC 文件组传输，双方实际发送/接收、唯一验证和提交均为 12,582,949 bytes，摘要一致；Mac 端拒绝、冲突、权限拒绝和独立磁盘镜像 ENOSPC 也已 PASS。该现场使用的是当时 `0.1.0` dirty snapshot；当前产品为 `v0.4.0` 测试预发布，但恢复/暂停新增能力与 Release 包尚未在同一物理矩阵重跑。该结果是混合有线/无线同站点实测，不等价于所有 LAN、跨公网、网络切换或双 NAT。
