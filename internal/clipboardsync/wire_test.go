...
func TestLeaseAndEventWireRoundTrip(t *testing.T) {
	...
	event := Event{LeaseID: "lease", OriginID: "peer", Boot: "boot", OriginSeq: 2, Lamport: 4, Kind: Text, OSGeneration: 9, Payload: []byte("hello")}
	event.Digest = eventDigest(event)
	message := Message{Type: "event", LeaseID: event.LeaseID, SessionID: "session", Generation: 3, OriginID: event.OriginID, Boot: event.Boot, OriginSeq: event.OriginSeq, Lamport: event.Lamport, Kind: event.Kind, Digest: event.Digest, OSGeneration: event.OSGeneration, PayloadBytes: uint32(len(event.Payload))}
	wire.Reset()
...
func TestWireRejectsOversizeTruncatedAndReboundEvent(t *testing.T) {
	...
	event := Event{LeaseID: "old", OriginID: "peer", Boot: "boot", OriginSeq: 1, Lamport: 1, Kind: Text, OSGeneration: 2, Payload: []byte("body")}
	event.Digest = eventDigest(event)
	message := Message{Type: "event", LeaseID: "new", SessionID: "session", Generation: 1, OriginID: event.OriginID, Boot: event.Boot, OriginSeq: event.OriginSeq, Lamport: event.Lamport, Kind: event.Kind, Digest: event.Digest, OSGeneration: event.OSGeneration, PayloadBytes: uint32(len(event.Payload))}
