...
- 我的设备、别名/固定/专属目录；本地屏蔽与所有接入/恢复路径共用授权。
- 持久发送草稿与顺序队列；拖放、Windows SendTo、Mac Finder Services和按profile隔离的单实例。
- 可选后台常驻、通知与活跃传输防睡眠，真正退出可保留恢复状态。
- 接收前选择清单、目录、冲突和子集；全部跳过有独立结果，默认不覆盖。
- 显式文字、链接、原生剪贴板图片快照；经认证QUIC传输，接收后手动复制、打开或另存。
- 已合入分页收件箱，支持检索、定位和清理记录；M6完整联调本轮不继续。

实际结果、失败及环境限制见 [M5执行记录](docs/evidence/DESKTOP-M5-EXECUTION.md)。这些功能不代表所有平台菜单、通知点击或网络环境已通过验收。
...
需要 Go 1.27.1。核心命令不依赖 Node 或桌面系统库。
